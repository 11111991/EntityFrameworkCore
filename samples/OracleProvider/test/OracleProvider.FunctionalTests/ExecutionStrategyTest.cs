...
namespace Microsoft.EntityFrameworkCore
{
    public class ExecutionStrategyTest : IClassFixture<ExecutionStrategyTest.ExecutionStrategyFixture>
    {
        ...
        private void Test_commit_failure(bool realFailure, Action<TestOracleRetryingExecutionStrategy, ExecutionStrategyContext> execute)
        {
            CleanContext();

            using (var context = CreateContext())
            {
                var connection = (TestOracleConnection)context.GetService<IOracleConnection>();

                connection.CommitFailures.Enqueue(new bool?[] { realFailure });

                context.Products.Add(new Product());
                execute(new TestOracleRetryingExecutionStrategy(context), context);
        ...
        private async Task Test_commit_failure_async(bool realFailure, Func<TestOracleRetryingExecutionStrategy, ExecutionStrategyContext, Task> execute)
        {
            CleanContext();
            ...
            using (var context = CreateContext())
            {
                ...
                context.Products.Add(new Product());
                await execute(new TestOracleRetryingExecutionStrategy(context), context);
                context.ChangeTracker.AcceptAllChanges();
        ...
        private void Test_commit_failure_multiple_SaveChanges(bool realFailure)
        {
            ...
            {
                var connection = (TestOracleConnection)context1.GetService<IOracleConnection>();
                ...
                using (var context2 = CreateContext())
                {
                    ...
                    new TestOracleRetryingExecutionStrategy(context1).ExecuteInTransaction(
                        context1,
                        c1 =>
                            {
        ...
        private void Test_execution_failure(bool realFailure)
        {
            ...
            using (var context = CreateContext())
            {
                ...
                context.Products.Add(new Product());
                context.Products.Add(new Product());
                new TestOracleRetryingExecutionStrategy(context).ExecuteInTransaction(
                    context,
                    c => c.SaveChanges(acceptAllChangesOnSuccess: false),
                    c =>
                        {
        ...
        public void Verification_is_retried_using_same_retry_limit()
        {
            ...
            using (var context = CreateContext())
            {
                ...
                Assert.Throws<RetryLimitExceededException>(
                    () =>
                        new TestOracleRetryingExecutionStrategy(context, TimeSpan.FromMilliseconds(100))
        ...
        public class ExecutionStrategyFixture : SharedStoreFixtureBase<DbContext>
        {
            ...
            protected override IServiceCollection AddServices(IServiceCollection serviceCollection)
            {
                return base.AddServices(serviceCollection)
                    .AddScoped<IOracleConnection, TestOracleConnection>()
            ...
            public override DbContextOptionsBuilder AddOptions(DbContextOptionsBuilder builder)
            {
                ...
                new OracleDbContextOptionsBuilder(options).MaxBatchSize(1);
                return options;
            }
        }
    }
